...
Let \(\mathbf{X}\) be a 3D scene point, let \(\mathbf{x}\) be its homogeneous image projection, let \(K\) be the intrinsic matrix, and let \([R \mid t]\) be the camera extrinsic matrix. The camera is modeled with the pinhole projection equation
\[
\mathbf{x} \sim K [R \mid t] \mathbf{X},
\]
where \(\sim\) denotes equality up to scale. In the implemented calibration, zero skew is assumed. Let \(f_x\) and \(f_y\) be the focal lengths in pixel units, and let \((c_x,c_y)\) be the principal point. The intrinsic matrix has the form
\[
K =
\begin{bmatrix}
f_x & 0 & c_x \\
...
\end{bmatrix}.
\]
Since no checkerboard sequence is available, the calibration is obtained from scene geometry. The method exploits the fact that 3D parallel lines are projected to image lines that meet at a common vanishing point. Let \(\mathbf{v}_i\) and \(\mathbf{v}_j\) be the vanishing points associated with two orthogonal 3D directions, and let \(\omega\) be the image of the absolute conic, defined from the intrinsic matrix as \(K^{-\top}K^{-1}\). The orthogonality constraint is
\[
...
The user then groups the detected segments according to the 3D direction they represent. Each group must contain at least two image segments that are parallel in the real scene. In the current configuration, three main line groups are selected, plus one additional upper group. For a segment with homogeneous endpoints \(\mathbf{p}_1\) and \(\mathbf{p}_2\), let \(\ell\) be the corresponding homogeneous image line. It is computed as
\[
\ell = \mathbf{p}_1 \times \mathbf{p}_2.
\]
For each selected group, all line equations are stacked in a matrix \(L\). Let \(\mathbf{v}\) be the vanishing point of that group. It is estimated as the least-squares intersection of the lines by solving
\[
L\mathbf{v} = 0
\]
...
After the vanishing points have been estimated, the selected directions are treated as orthogonal constraints. Let \((u_i,v_i)\) and \((u_j,v_j)\) be the image coordinates of two vanishing points associated with orthogonal directions. With the principal point \((c_x,c_y)\) fixed at the image center and with non-square pixels allowed, the orthogonality equation becomes
\[
...
+ 1 = 0.
\]
By defining \(a_x = 1/f_x^2\) and \(a_y = 1/f_y^2\), each orthogonal pair gives a linear equation in \(a_x\) and \(a_y\). The implementation solves this overdetermined system with least squares and then recovers \(f_x\) and \(f_y\). The current calibration output is saved in \texttt{outputGeometry/calibration\_K.json} and gives
\[
K =
\begin{bmatrix}
1781.14 & 0 & 960.00 \\
...
The pairwise transformations are normalized as \(3 \times 3\) homographies and composed along the camera interval. Let \(r\) be the reference frame, let \(\mathbf{p}_t\) be a point measured in frame \(t\), let \(H_{t \rightarrow r}\) be the cumulative transform from frame \(t\) to frame \(r\), and let \(\tilde{\mathbf{p}}_t\) be the mapped point in the reference coordinate system. The mapping is
\[
...
Finally, the midpoint between the two reconstructed track borders defines the center plane used for the jumper trajectory. Let \(\mathbf{n}\) be the oriented normal between the two track borders, let \(w\) be the reconstructed track width, and let \(\mathbf{P}^A_1,\ldots,\mathbf{P}^A_N\) be the reconstructed points on one border. The implementation takes the first point of this border as \(\mathbf{O}_A=\mathbf{P}^A_1\) and defines the corresponding origin on the parallel border as
\[
\mathbf{O}_B = \mathbf{O}_A + w\mathbf{n}.
\]
The origin of the center plane is the midpoint between these two origins:
\[
\mathbf{O}_c = \frac{\mathbf{O}_A+\mathbf{O}_B}{2}.
\]

The plane basis is derived from the reconstructed border direction. Let \(\mathbf{a}=\mathbf{P}^A_N-\mathbf{P}^A_1\) be the direction from the first to the last reconstructed point on the border. Since this vector can contain a small component along the normal, it is projected onto the plane orthogonal to \(\mathbf{n}\) and normalized:
\[
\mathbf{e}_1 =
\frac{\mathbf{a}-(\mathbf{a}^\top\mathbf{n})\mathbf{n}}
{\left\|\mathbf{a}-(\mathbf{a}^\top\mathbf{n})\mathbf{n}\right\|}.
\]
The second direction is chosen orthogonal to both the normal and \(\mathbf{e}_1\):
\[
\mathbf{e}_2 = \mathbf{n}\times\mathbf{e}_1.
\]
These two vectors define the coordinate axes on the center plane. Let \(H_{\text{plane}\rightarrow\text{image}}\) be the homography from center-plane coordinates to image coordinates. The homography is built as
\[
...
\end{bmatrix}.
\]
The inverse homography maps the stabilized reference-frame trajectory points into plane coordinates, and these coordinates are then converted into 3D points on the reconstructed center plane.
