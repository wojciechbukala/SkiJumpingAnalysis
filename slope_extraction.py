...
try:
    ...
    CALIBRATION_FRAME_PATH = Path("extracted_frames/G_30out_frame_00004.jpg")
...
def estimate_vanishing_point_from_segments(
    ...
    if len(segments) < MIN_NORMAL_SEGMENTS:
        raise ValueError(f"Servono almeno {MIN_NORMAL_SEGMENTS} segmenti normali.")
...
def reconstruct_curves_3d(
    ...
    for sign in (1.0, -1.0):
        ...
        left_arr = rays_a * depth_a[:, None]
        right_arr = rays_b * depth_b[:, None]
        residual_arr = (right_arr - left_arr) - float(width) * oriented_normal
        depth_arr = np.concatenate([depth_a, depth_b])
        positive_depth_ratio = float(np.mean(depth_arr > 0.0))
        mean_residual = float(np.mean(np.linalg.norm(residual_arr, axis=1)))
        candidates.append(
            (
                ...
                ReconstructionResult(
                    left_3d=left_arr,
                    right_3d=right_arr,
                    center_3d=0.5 * (left_arr + right_arr),
